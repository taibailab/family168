/**
 * cvi_glossy_lib.js 1.1 (16-Oct-2007)
...
//			}
	...
	modify: function(canvas, options) {
		try {			
			...
			var pressed = (typeof options['pressed']=='boolean'?options['pressed']:canvas.options['pressed']); canvas.options['pressed']=pressed;
			var highlight = (typeof options['highlight']=='boolean'?options['highlight']:canvas.options['highlight']); canvas.options['highlight']=highlight;
			var highlightcolor = '#e7cf04';
			var ihighlight = 0; if(isNaN(highlightcolor)) var ihighlight = (highlightcolor.match(/^#[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]$/i)?highlightcolor:0);
			...
			if(document.all && document.namespaces && !window.opera) {
				if(canvas.tagName.toUpperCase() == "DIV") {
					if(isNaN(icolor)) {
				...
				if(canvas.tagName.toUpperCase() == "CANVAS" && canvas.getContext("2d")) {
					...
					img.onload = function() {
						...
						context.save();
						roundedRect(context,is,0,iw-(is*2),ih,r);
						context.clip();
						if(isNaN(icolor)) {
							if(isNaN(icolor2)) {
								if(igradient=='h') {
								...
								if (highlight){
									context.fillStyle = ihighlight;
									context.fillRect(0,0,iw,ih+(is*2));
									context.fillStyle = icolor;
									context.fillRect(((os+is)*2),((os+is)*2),iw-(4*(os+is)),ih-(4*(os+is)));	
								}
								else{
									context.fillStyle = icolor;
									context.fillRect(0,0,iw,ih-(is*2));	
						...
						context.drawImage(img,is,0,iw-(is*2),ih-(is*4));
						if (pressed){
							addBright(context,os+is,os,iw-(2*(os+is)),(ih/2)-os,sr,ishade*1.25);
							addDark(context,os+is,(ih/2)-is,iw-(2*(os+is)),(ih/2)-((os+is)),sr,ishade*1.75);
						}
						else{
							addBright(context,os+is,os,iw-(2*(os+is)),(ih/2)-os,sr,ishade*1.5);
							addDark(context,os+is,(ih/2)-is,iw-(2*(os+is)),(ih/2)-((os+is)),sr,ishade);
						}
						if(noradius==false) addFrame(context,is,os,iw-(is*2),ih-(is*2),r,ishade*0.5)
						context.restore();
					}
